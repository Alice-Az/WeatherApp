...
const WeatherOverview = (props) => {
    ...
    const SaveLocation = () => {
        if(props.location !== null) {
            localStorage.setItem(`${props.location?.Key}`, `${JSON.stringify(props.location)}`);
            ...
            <div className='weather-overview'>
                <div className='weather-data'>
                    <div className='location'>
                        <p className='location-name'>{props.location?.LocalizedName}</p>
                        {!isSaved && !props.isLoading && <button className='save-location' onClick={SaveLocation}><FontAwesomeIcon className='save-location-icon' icon={starEmpty}/></button>}
                        {isSaved && !props.isLoading && <button className='save-location' onClick={UnSaveLocation}><FontAwesomeIcon className='save-location-icon' icon={starFull}/></button>}
                    </div>
