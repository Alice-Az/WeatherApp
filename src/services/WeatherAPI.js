...
export const GetLocations = async (input) => {
    let url =
        "https://dataservice.accuweather.com/locations/v1/cities/search?apikey=Ix64mBi5uAs2vwkVj5cXyWSFL0eKO3cz&q=" +
        input;
...
export const GetGeolocationResult = async (lat, long) => {
    let url = `https://dataservice.accuweather.com/locations/v1/cities/geoposition/search?apikey=Ix64mBi5uAs2vwkVj5cXyWSFL0eKO3cz&q=${lat}%2C${long}`;
...
export const GetWeather = async (locationKey) => {
    let url = `https://dataservice.accuweather.com/currentconditions/v1/${locationKey}?apikey=Ix64mBi5uAs2vwkVj5cXyWSFL0eKO3cz&details=true`;
...
export const GetForecastHourly = async (locationKey) => {
    let url = `https://dataservice.accuweather.com/forecasts/v1/hourly/12hour/${locationKey}?apikey=Ix64mBi5uAs2vwkVj5cXyWSFL0eKO3cz&metric=true`;
    ...
    return await fetch(url)
        ...
        .then((data) => {
            ...
            for (let i = 0; i < data.length; i++) {
                let newDT = `${data[i].DateTime.substring(0, 10)} - ${data[
                    i
...
export const GetForecastDaily = async (locationKey) => {
    let url = `https://dataservice.accuweather.com/forecasts/v1/daily/5day/${locationKey}?apikey=Ix64mBi5uAs2vwkVj5cXyWSFL0eKO3cz&metric=true`;
